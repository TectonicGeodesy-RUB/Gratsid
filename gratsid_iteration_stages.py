...
def do_initial_fit(x,y,err,options,known_steps):
    perm_steps_list = generate_permanent_step_list(x,known_steps,options) #<---- should max gap be for the sampling or the time?
...
def initialize_before_while(residual_val):
    ...
    trans_table = np.array([]).reshape(0,3)  ## instantiating the table that will carry the changing list of transient functions
    previous_trans_table = np.array([]).reshape(0,3)  ## instantiating the table that will carry the changing list of transient functions
...
def find_new_transient_onsets(residual,quarantine_list,options,\
    ...
    G_TOs = make_TO_G_lists(options=options,x=x,TO_list=TO_list)## with this python list making lists of greens functions lists depending on how many types of transients allowed
...
def combinatorial_analysis(previous_trans_table,candidates_table,\
    ...
    for i in range(trans_table.shape[0]):
        G_trans_list.append(assemble_G(x,trans_table[i,:][None,:],options)) # do we really need to repeat on each iteration? Can we rather save this?
    ...
    remove_n = 1
    remove_list = (np.arange(options['max_remove'])*np.nan)[None,:] # first row corresponds to none of the TOs being removed (all being kept)
    ...
    for i in range(widths_unique.size):
        ...
        for j in range(T_new.shape[0]):
            to_remove = np.array(trans_indices)\
